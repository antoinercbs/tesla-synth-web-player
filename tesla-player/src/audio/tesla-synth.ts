...
import { envelopeAmplitude, envelopeReleaseMs, programSteps } from '@/sysex/envelopes';
...
const D_MIN = 0.04, D_MAX = 0.26; // clamp on the audio duty (≈B2..C6)
...
const FORMANT_HZ = 2600, FORMANT_DB = 2.5, FORMANT_Q = 0.9; // gentle measured ~2.5 kHz body resonance
...
const FLICKER_DEPTH = 0.18, FLICKER_STEP_MS = 38, FLICKER_HORIZON_S = 2.2;

interface Stamped { when: number; v: number }
interface CoilState { mask: number; ontime: Stamped[]; duty: Stamped[] }
interface Voice {
...
function noteHz(note: number): number { return 440 * 2 ** ((note - 69) / 12); }
...
export class TeslaSynthOutput implements MidiSink {
  ...
  constructor() {
    ...
    formant.type = 'peaking'; formant.frequency.value = FORMANT_HZ; formant.gain.value = FORMANT_DB; formant.Q.value = FORMANT_Q;
    const comp = this.ctx.createDynamicsCompressor();
    comp.threshold.value = -12; comp.ratio.value = 8; comp.attack.value = 0.002; comp.release.value = 0.18;
    this.master = this.ctx.createGain();
    this.master.gain.value = 0.34;
    this.bus.connect(formant).connect(comp).connect(this.master).connect(this.ctx.destination);
  ...
  private pulseFor(note: number): PeriodicWave {
    ...
    const n = Math.max(1, Math.min(PULSE_HARMONICS_MAX, Math.floor((this.ctx.sampleRate / 2) / hz) - 1));
    const real = new Float32Array(new ArrayBuffer((n + 1) * 4));
    const imag = new Float32Array(new ArrayBuffer((n + 1) * 4));
    for (let h = 1; h <= n; h++) imag[h] = (2 / (h * Math.PI)) * Math.sin(h * Math.PI * d);
  ...
  send(data: number[], options?: { time?: number | string }): void {
    ...
    const status = typeof data[0] === 'number' ? data[0] : parseInt(String(data[0]).replace(/^0x/i, ''), 16);
    const kind = status & 0xf0;
    const ch = status & 0x0f;
    if (kind === 0x90 && data[2] > 0) this.noteOn(ch, data[1], data[2], when);
    else if (kind === 0x80 || (kind === 0x90 && data[2] === 0)) this.noteOff(ch, data[1], when);
  ...
  sendAllSoundOff(): void {
    for (const v of [...this.live]) this.endVoice(v, this.ctx.currentTime, 0.01);
    this.voices.clear();
  }

  clear(): void {
    for (const v of [...this.live]) this.disposeVoice(v, this.ctx.currentTime, 0);
    this.live = [];
    this.voices.clear();
    this.coils = [];
  ...
  private applyFrame(frame: number[], when: number): void {
    let dec;
    try { dec = decodeFrame(frame); } catch { return; }
    const c = dec.coil;
    if (c < 0 || c > 5) return;
  ...
  private stampAt(list: Stamped[], when: number, fallback: number): number {
    let v = fallback; let best = -Infinity;
    for (const s of list) if (s.when <= when + 1e-4 && s.when >= best) { best = s.when; v = s.v; }
    return v;
  }
  private channelParams(ch: number, when: number): { ontime: number; duty: number } {
    for (let c = 0; c < this.coils.length; c++) {
      const cs = this.coils[c];
      if (cs && (cs.mask & (1 << ch)) !== 0) {
        return { ontime: this.stampAt(cs.ontime, when, 0), duty: this.stampAt(cs.duty, when, 0) };
      }
    }
   ...
   */
  private scheduleAttack(g: AudioParam, program: number, velScale: number, when: number): void {
    const a0 = envelopeAmplitude(program, 0, null) * velScale;
    g.setValueAtTime(0.0001, when);
    let startMs = 0;
    if (a0 > 0.001) { g.linearRampToValueAtTime(a0, when + ATTACK_MIN_S); startMs = ATTACK_MIN_S * 1000; }
    ...
    for (let guard = 0; guard < 16; guard++) {
      ...
      if (dur > 0) {
        ...
        for (let k = 1; k <= m; k++) {
          ...
          if (tMs > startMs) {
            g.linearRampToValueAtTime(Math.max(0.0001, envelopeAmplitude(program, tMs, null) * velScale), when + tMs / 1000);
          }
        }
        acc += dur;
      }
  ...
  private noteOn(ch: number, note: number, vel: number, when: number): void {
    if (this.live.length >= MAX_VOICES) {
      ...
      if (st) { this.voices.delete(st.key); this.disposeVoice(st, when, 0.01); }
    }
    ...
    const loud = ctx.createGain(); loud.gain.setValueAtTime(energy, ctx.currentTime);
    // ~Butterworth low-pass (no resonance) — tames the over-clean upper sinc lobes.
    const filter = ctx.createBiquadFilter();
    filter.type = 'lowpass'; filter.frequency.value = FILTER_HZ; filter.Q.value = 0.7;
    ...
    source.frequency.value = noteHz(note) * (1 + (Math.random() * 2 - 1) * PITCH_JITTER);
    source.connect(env);
    ...
    const voice: Voice = { source, env, flick, loud, filter, ch, program, velScale, startWhen: when, key };
  ...
  private noteOff(ch: number, note: number, when: number): void {
    ...
    try {
      ...
      for (let i = 1; i <= n; i++) {
        const rt = (i / n) * relMs;
        const amp = envelopeAmplitude(v.program, heldElapsed + rt, heldElapsed) * v.velScale;
        v.env.gain.linearRampToValueAtTime(Math.max(0.0001, amp), when + rt / 1000);
      }
    } catch { /* */ }
  ...
  private disposeVoice(v: Voice, when: number, tailS: number): void {
    try { v.source.stop(when + tailS); } catch { /* already stopped */ }
    const i = this.live.indexOf(v);
    if (i !== -1) this.live.splice(i, 1);
    const delayMs = Math.max(0, (when - this.ctx.currentTime + tailS) * 1000) + 40;
    setTimeout(() => {
      try { v.source.disconnect(); } catch { /* */ }
      try { v.env.disconnect(); } catch { /* */ }
      try { v.flick.disconnect(); } catch { /* */ }
      try { v.loud.disconnect(); } catch { /* */ }
  ...
  private holdAt(g: AudioParam, when: number): void {
    ...
    else { g.cancelScheduledValues(when); g.setValueAtTime(Math.max(0.0001, g.value), when); }
  }
  ...
  private remodulate(coil: number, when: number): void {
    ...
    if (!cs) return;
    const target = TeslaSynthOutput.energy(this.stampAt(cs.ontime, when, 40), this.stampAt(cs.duty, when, 0.05));
    for (const v of this.live) {
      if ((cs.mask & (1 << v.ch)) === 0) continue;
      try {
        this.holdAt(v.loud.gain, when);
        v.loud.gain.linearRampToValueAtTime(Math.max(0.0001, target), when + 0.03);
      } catch { /* */ }
    }
  }
}
