...
const DEFAULT_SYSEX = () => [{ command: 'set enable for mode midi-live to 1', value: 'f0 00 26 05 01 7f 20 00 00 02 01 00 00 00 00 f7' }];
const ALL_CHANNELS = () => [true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true];
...
export default {
    data: function() {
        return {
            ...
            sysex: DEFAULT_SYSEX(),
            selectedMidiFileId: "",
            outputMapping1Boolean: ALL_CHANNELS(),
            outputMapping2Boolean: ALL_CHANNELS(),
        }
    },
    computed: {
        ...
        selectedFilePath() {
            const file = this.midiStore.midiFileList.find(e => e.id == this.selectedMidiFileId);
    ...
    methods: {
        resetSong() {
            ...
            this.sysex = DEFAULT_SYSEX();
            this.selectedMidiFileId = "";
            this.outputMapping1Boolean = ALL_CHANNELS();
            this.outputMapping2Boolean = ALL_CHANNELS();
        ...
        loadSong(song) {
            ...
            if (song.midiFile && song.midiFile.id) {
                this.selectedMidiFileId = song.midiFile.id;
            }
            if (song.sysex && song.sysex.length > 0 && song.sysex[0].value) {
        ...
        buildPayload() {
            return {
                ...
                midiFileId: this.selectedMidiFileId === "" ? null : this.selectedMidiFileId,
                id: this.id
            };
        },
        saveSong() {
            ...
            if (this.id) {
                this.axios.put(`/api/songs/${this.id}`, data).then(response => {
        ...
        sendToPlayer() {
            const data = {
                ...
                outputMapping1: this.outputMapping1,
                outputMapping2: this.outputMapping2,
                midiFile: this.midiStore.midiFileList.find(midi => midi.id == this.selectedMidiFileId),
                id: this.id
            }
