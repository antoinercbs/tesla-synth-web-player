        ...
        <ul class="pl-list">
          <li v-for="s in filteredLibrary" :key="s.id" class="pl-row"
            ...
            <span v-else class="coil-dots">
              <span v-for="i in coilChips(s.coilCount)" :key="i" class="coil-dot"
...
.pl-meta__field--name .text-field {
  width: 100%;
}
...
.pl-list--drop.is-drop {
  outline: 2px dashed var(--volt);
  outline-offset: -4px;
}
