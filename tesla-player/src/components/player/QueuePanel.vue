  ...
  <article class="play-panel queue-panel">
    <header class="play-panel__head queue-head">
      ...
      <div class="queue-toggles">
        <button class="row-btn" type="button" :class="{ 'is-active': shuffle }" @click="emit('toggle-shuffle')"
          :title="$t('label.shuffle')">
          <i class="fas fa-shuffle"></i>
        </button>
        <button class="row-btn" type="button" :class="{ 'is-active': repeat !== 'off' }" @click="emit('cycle-repeat')"
...
.queue-item.is-current .queue-item__name {
  color: var(--volt);
  font-weight: 600;
}
